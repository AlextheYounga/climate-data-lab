...
def collect_key_stats(filepath):
    """
    ...
    2. Break list of points into chunks based on exponential or linear scales, (see exponential_scales() function).
    ...
    """
    data_points = readJson(filepath)
    if (data_points):
        points = list(reversed(extract_data(data_points, 'Mean', queryKey=['Source', 'GCAG'])))
        ...
        returns = returns_calculator(points)
...
def fractal_calculator(dataset, output='table'):
    """
    ...
    """
    filepath = data_route(dataset)
    scales, range_stats = collect_key_stats(filepath)
